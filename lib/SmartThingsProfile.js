...
function addpat(nodeId,pat,isRefresh){
	if(!pat){
		_MDinfos[nodeId]=Promise.resolve(null);
		return Promise.resolve(null);
	}
	if(!_MDinfos[nodeId]||isRefresh){
        const PAT = {};
        let failed = [];
		...
		_MDinfos[nodeId]=getLocationsPR
            ...
            .then((results)=>{
                ...
                if(PAT.devices){
					_MDinfos[nodeId]=Promise.resolve(PAT);
                }else{
					_MDinfos[nodeId]=Promise.resolve(null);
                }
                return _MDinfos[nodeId];
            })
    }else if(_MDinfos[nodeId]){
        return _MDinfos[nodeId];
    }
}
function getMDinfos(){
	const result = {};
	return Promise.all(
		Object.keys(_MDinfos).map(nodeId=>_MDinfos[nodeId].then(info=>{result[nodeId]=info}))
		).then(()=>{return result});
}
module.exports = {
    getCapabilities:getCapabilities,
    addpat:addpat,
	getMDinfos:getMDinfos
}