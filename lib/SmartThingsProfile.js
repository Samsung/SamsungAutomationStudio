...
fs.readFile('./lib/SmartThingsCapabilities.json', (err, data) => {
    if (err) throw err;
...
function PATremoveNodeId(nodeId){
    Object.values(_PATs).filter(PAT=>PAT.nodeIds.includes(nodeId))
                        .forEach(PAT=>{PAT.nodeIds = PAT.nodeIds.splice(PAT.nodeIds.indexOf(nodeId),1)});
}
function clearUnusedPAT(){
    Object.keys(_PATs).forEach(pat=>{
        const PAT = _PATs[pat];
        if(!(PAT.nodeIds.length>0)){
            delete _PATs[pat];
        }
    })
}
function addpat(nodeId,pat,isRefresh){
    ...
    if(!_PATs[pat]||isRefresh){
        ...
        return Promise.resolve(_PATs[pat]).then(PAT=>{
            if(PAT.nodeIds&&PAT.nodeIds.indexOf(nodeId)>-1){
                PATremoveNodeId(nodeId);
                PAT.nodeIds.push(nodeId);
                clearUnusedPAT();
            }
...
function getPATs(){
    ...
    if(PRs.length>0){
        return Promise.allSettled(PRs).then(results=>results.map(result=>result.value).reduce((PATs,PAT)=>Object.assign(PATs,PAT)))
    }else{
        return Promise.resolve(null);
    }
}
